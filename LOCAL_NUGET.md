...
The script discovers packable projects under `src`, runs `dotnet pack`, and writes the generated `.nupkg` files to the default repository-local feed:
...
powershell -ExecutionPolicy Bypass -File .\Publish-LocalNuGet.ps1 `
  ...
  -Project .\src\CodeQuality\CodeQuality.csproj
```
