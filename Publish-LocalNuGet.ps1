...
if (-not (Test-Path (Join-Path $repoRoot "CodeQuality.sln"))) {
    $parentRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path

    if (Test-Path (Join-Path $parentRoot "CodeQuality.sln")) {
        $repoRoot = $parentRoot
    }
    else {
        throw "Unable to locate CodeQuality.sln from script path '$PSScriptRoot'."
    }
}
